...
WIFI_FAILURE_LOG_INTERVAL = 30.0
...
def connect_wifi_socket(
    host: str,
    port: int,
    *,
    last_failure_log: float = 0.0,
    suppress_failures: bool = False,
) -> Tuple[Optional[socket.socket], float, bool]:
    if not WIFI_ENABLED or not host:
        return None, last_failure_log, suppress_failures
    try:
        print(f"Trying Wi-Fi monitor at {host}:{port}...")
        sock = socket.create_connection((host, port), timeout=3.0)
        sock.settimeout(2.0)
        print(f"Connected to Arduino over Wi-Fi at {host}:{port}")
        return sock, 0.0, False
    except OSError as exc:
        now = time.time()
        should_log = (not suppress_failures) or ((now - last_failure_log) >= WIFI_FAILURE_LOG_INTERVAL)
        if should_log:
            print(f"Wi-Fi unavailable at {host}:{port}: {exc}")
...
def choose_transport(
    active_serials: Optional[Dict[str, serial.Serial]],
    active_wifi: Optional[socket.socket],
    *,
    wifi_failure_log_ts: float = 0.0,
    wifi_failure_suppressed: bool = False,
) -> Tuple[Dict[str, serial.Serial], Optional[socket.socket], str, float, bool]:
    serials = connect_arduinos(active_serials or {})
    if serials and PREFER_USB:
        if active_wifi is not None:
            print("USB detected. Switching transport from Wi-Fi back to USB.")
            close_socket(active_wifi)
            active_wifi = None
        return serials, active_wifi, "usb", 0.0, False

    if serials and not PREFER_USB:
        return serials, active_wifi, "usb", wifi_failure_log_ts, wifi_failure_suppressed

    if active_wifi is None:
        active_wifi, wifi_failure_log_ts, wifi_failure_suppressed = connect_wifi_socket(
            WIFI_HOST,
            WIFI_PORT,
            last_failure_log=wifi_failure_log_ts,
            suppress_failures=wifi_failure_suppressed,
        )
    if active_wifi is not None:
        return serials, active_wifi, "wifi", 0.0, False

    return serials, active_wifi, "none", wifi_failure_log_ts, wifi_failure_suppressed


def send_to_active_transport(payload: str, arduino_serials: Dict[str, serial.Serial], wifi_sock: Optional[socket.socket], active_transport: str) -> Tuple[Dict[str, serial.Serial], Optional[socket.socket], str]:
    if active_transport == "usb" and arduino_serials:
        ...
        if arduino_serials:
            return arduino_serials, wifi_sock, "usb"
        print("USB transport lost.")
        active_transport = "none"

    if active_transport == "wifi" and wifi_sock is not None:
        try:
            wifi_sock.sendall(payload.encode("utf-8"))
            return arduino_serials, wifi_sock, "wifi"
        except OSError as exc:
    ...
    return arduino_serials, wifi_sock, active_transport
...
def main() -> None:
    ...
    print(f"Preferred transport: {'USB -> Wi-Fi fallback' if PREFER_USB else 'Wi-Fi -> USB fallback'}")
    if WIFI_ENABLED:
    ...
    wifi_failure_log_ts = 0.0
    wifi_failure_suppressed = False
    ...
    while True:
        try:
            ...
            should_probe_usb = (now - last_discovery_attempt) >= RETRY_DELAY or active_transport != "usb"
            should_probe_wifi = WIFI_ENABLED and (wifi_sock is None) and ((now - last_wifi_attempt) >= WIFI_RETRY_DELAY)

            if should_probe_usb or should_probe_wifi or active_transport == "none":
                if should_probe_usb:
                    last_discovery_attempt = now
                if should_probe_wifi:
                    last_wifi_attempt = now
                arduino_serials, wifi_sock, chosen, wifi_failure_log_ts, wifi_failure_suppressed = choose_transport(
                    arduino_serials,
                    wifi_sock,
                    wifi_failure_log_ts=wifi_failure_log_ts,
                    wifi_failure_suppressed=wifi_failure_suppressed,
                )
                if chosen != active_transport and chosen != "none":
                    print(f"Active transport: {chosen.upper()}")
                active_transport = chosen

            arduino_serials, wifi_sock, active_transport = send_to_active_transport(
                payload, arduino_serials, wifi_sock, active_transport
            )
