...
def connect_wifi_socket(host: str, port: int) -> Optional[socket.socket]:
...
def choose_transport(active_serials: Optional[Dict[str, serial.Serial]], active_wifi: Optional[socket.socket]) -> Tuple[Dict[str, serial.Serial], Optional[socket.socket], str]:
    ...
    if serials and PREFER_USB:
        if active_wifi is not None:
            ...
            active_wifi = None
        return serials, active_wifi, "usb"

    if serials and not PREFER_USB:
        return serials, active_wifi, "usb"

    if active_wifi is None:
        active_wifi = connect_wifi_socket(WIFI_HOST, WIFI_PORT)
    if active_wifi is not None:
        return serials, active_wifi, "wifi"

    return serials, active_wifi, "none"
...
def main() -> None:
    ...
    while True:
        try:
            ...
            should_probe_usb = (now - last_discovery_attempt) >= RETRY_DELAY
            ...
            if should_probe_usb or should_probe_wifi or active_transport == "none":
                ...
                arduino_serials, wifi_sock, chosen = choose_transport(arduino_serials, wifi_sock)
