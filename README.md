...
- When **multiple** Arduino UNO R3 boards are detected, the flasher asks whether those boards have the **2.8"** or **3.5"** TFT shield before uploading, because USB detection alone cannot distinguish the attached screen size. With a single detected UNO R3, the flasher now defaults to the **2.8"** sketch unless you override it with `UNO_R3_SCREEN_SIZE=35`. You can also preselect either screen size non-interactively with `UNO_R3_SCREEN_SIZE=28 ./install_arduinos.sh` or `UNO_R3_SCREEN_SIZE=35 ./install_arduinos.sh`. The Java Control Center shows this selection in a GUI popup instead of relying on terminal input.
