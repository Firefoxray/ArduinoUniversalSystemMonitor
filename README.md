...
- When an Arduino UNO R3 is detected, the flasher now asks whether that board has the **2.8"** or **3.5"** TFT shield before uploading, because USB detection alone cannot distinguish the attached screen size. You can also preselect it non-interactively with `UNO_R3_SCREEN_SIZE=28 ./install_arduinos.sh` or `UNO_R3_SCREEN_SIZE=35 ./install_arduinos.sh`.
