...
static void drawCurrentLayout() {
  drawHeader(F("Ray Co. Universal System Monitor"), currentPage + 1);
}
...
static void parseIncomingLine(char* s) {
  ...
  ramPct   = parsePercentField(fields[5]);
  diskPct  = parsePercentField(fields[6]);
  disk1Pct = parsePercentField(fields[7]);

  safeCopy(cpuTemp, sizeof(cpuTemp), fields[8]);
  safeCopy(osName, sizeof(osName), fields[9]);
  safeCopy(hostName, sizeof(hostName), fields[10]);
  safeCopy(ipAddr, sizeof(ipAddr), fields[11]);
  safeCopy(uptimeStr, sizeof(uptimeStr), fields[12]);
  safeCopy(downStr, sizeof(downStr), fields[13]);
  safeCopy(upStr, sizeof(upStr), fields[14]);
  safeCopy(cpuFreqStr, sizeof(cpuFreqStr), fields[17]);

  gpuPct      = parsePercentField(fields[18]);
  safeCopy(gpuTemp, sizeof(gpuTemp), fields[19]);
  gpuMemUsed  = (uint16_t)atoi(fields[20]);
  gpuMemTotal = (uint16_t)atoi(fields[21]);
  gpuMemPct   = parsePercentField(fields[22]);
  gpuClock    = (uint16_t)atoi(fields[23]);
  safeCopy(gpuName, sizeof(gpuName), fields[24]);

  safeCopy(proc1, sizeof(proc1), fields[25]);
  safeCopy(proc2, sizeof(proc2), fields[26]);
  safeCopy(proc3, sizeof(proc3), fields[27]);
  safeCopy(proc4, sizeof(proc4), fields[28]);

  safeCopy(proc1Cpu, sizeof(proc1Cpu), fields[31]);
  safeCopy(proc1Ram, sizeof(proc1Ram), fields[32]);
  safeCopy(proc2Cpu, sizeof(proc2Cpu), fields[33]);
  safeCopy(proc2Ram, sizeof(proc2Ram), fields[34]);
  safeCopy(proc3Cpu, sizeof(proc3Cpu), fields[35]);
  safeCopy(proc3Ram, sizeof(proc3Ram), fields[36]);
  safeCopy(proc4Cpu, sizeof(proc4Cpu), fields[37]);
  safeCopy(proc4Ram, sizeof(proc4Ram), fields[38]);
