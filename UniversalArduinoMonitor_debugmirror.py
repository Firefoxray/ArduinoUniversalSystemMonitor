#!/usr/bin/env python3
"""Universal Linux Arduino system monitor sender v7.3-debugmirror.
...
- Adds an optional debug mirror serial output for the Java fake display

Debug mirror:
- Set ARDUINO_MONITOR_DEBUG_PORT=/tmp/fakearduino_in
- The normal Arduino still receives the original positional payload
- The debug mirror receives KEY:VALUE packets for the Java fake display
"""
...
DEBUG_MIRROR_PORT = os.environ.get("ARDUINO_MONITOR_DEBUG_PORT", "").strip()
DEBUG_MIRROR_BAUD = int(os.environ.get("ARDUINO_MONITOR_DEBUG_BAUD", str(BAUD)))
DEBUG_MIRROR_ENABLED = bool(DEBUG_MIRROR_PORT)
...
def main() -> None:
    static = get_cached_static()
    print("Running Universal Arduino Monitor 7.3 debug mirror for Linux")
